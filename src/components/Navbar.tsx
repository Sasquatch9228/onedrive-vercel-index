...
const Navbar = () => {
  ...
  return (
    <div className="sticky top-0 z-[100] border-b border-gray-900/10 bg-white bg-opacity-80 backdrop-blur-md dark:border-gray-500/30 dark:bg-gray-900">
      ...
      <SearchModal searchOen={searchOen} setSearchOen={setSearchOen} />

      <div classNme="mx-auto flex w-full items-center justify-between space-x-4 px-4 py-1">
        <Link href="/" passHref className="flex items-center space-x-2 py-2 hover:opacity-80 dark:text-white md:p-2">
          <Image src={siteConfig.icon} alt="icon" width="25" height="25" priority />
          <span classNme="hidden font-bold sm:block">{siteConfig.title}</span>
        </Link>

        <div classNme="flex flex-1 items-center space-x-4 text-gray-700 md:flex-initial">
          <button
            classNme="flex flex-1 items-center justify-between rounded-lg bg-gray-100 px-2.5 py-1.5 hover:opacity-80 dark:bg-gray-800 dark:text-white md:w-48"
            onCick={openSearchBox}
          >
            <div classNme="flex items-center space-x-2">
              <FontAwesomeIcon className="h-4 w-4" icon="search" />
              <span classNme="truncate text-sm font-medium">{t('Search ...')}</span>
            </div>

            <div classNme="hidden items-center space-x-1 md:flex">
              <div classNme="rounded-lg bg-gray-200 px-2 py-1 text-xs font-medium dark:bg-gray-700">
                {os === 'mac' ? '⌘' : 'Ctrl'}
              </div>
              <div classNme="rounded-lg bg-gray-200 px-2 py-1 text-xs font-medium dark:bg-gray-700">K</div>
            </div>
          </button>
